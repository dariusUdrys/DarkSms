...
init(autoreset =True)
def tel3():
    leads_file =open("./cache/"+"fn_3.txt","r")
    path =os.getcwd()
    hlr_lookup =pathlib.Path(path ,"HLR Lookup")
    hlr_lookup.mkdir(parents =True ,exist_ok =True)
    def lookUp():
        leadsfile =open("./DL Proxy/"+"ec3.txt","r")
        while True:
            proxy =leadsfile.readline()
            if not proxy:
                leadsfile.close()
                time.sleep(32)
                return lookUp()
            try:
                ...
                ipCheck =requests.get(ip_url ,proxies ={"http":proxy ,"https":proxy },timeout =4)
                time.sleep(0.5)
                ipAddr =requests.get(ip_url ,proxies ={"http":proxy ,"https":proxy },timeout =4)
                ...
                for i in range(15):
                    ...
                    if response.status_code == 429 or response.text == '':
                        ...
                        if not carrier =="":
                            ...
                            directory =hlr_lookup+"/"+carrierFile 
                            workingFile =open(directory ,"a")
                            workingFile.writelines(number)
                            workingFile.close()
                        elif carrier =="":
